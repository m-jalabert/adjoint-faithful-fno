...
Entry points::
    ...
    python -m turbfno.anomaly finalize  --contract config/...json
    python -m turbfno.anomaly preflight --contract config/...json
    python -m turbfno.anomaly run       --contract config/...json
"""
...
from .dataset import DATASET_VERSION, GRID_SHAPE, TRAIN_RANGE, western_boundary_mask
from .objective import WESTERN_BOUNDARY_WIDTH

#: Report key for the western-boundary band. Derived from the width the
#: diagnostic actually uses: at 1 degree that band was four cells, and at
#: 0.25 degrees it is sixteen for the same four degrees of ocean. A literal
#: here would quietly describe a different band from the one measured.
WESTERN_BAND_KEY = f"western_first_{WESTERN_BOUNDARY_WIDTH}_wet_cells"
from .diagnostics import derived_fields
from .model import MANIFEST_NAME, README_NAME
from .plots import (
    FIGURE_3_LEADS,
    FIGURE_7_LEADS,
    STREAMFUNCTION_BOUND_SV,
    _finite_bound,
    _masked,
    _style,
)

VERSION = "turb_forward_control_v1_s0_turb_anomaly_v1"
...
FIGURE_3A = "turb_figure3a_streamfunction_anomaly_025deg_s0turb_dt10.png"

FIGURE_7A = "turb_figure7a_streamfunction_anomaly_day060_day2000_s0turb.png"

FIGURE_NAMES = (FIGURE_3A, FIGURE_7A)

REFERENCE_FIGURE = "turb_reference_time_mean_streamfunction_s0turb.png"

REPORT_NAME = "turb_s0turb_anomaly_report.json"

ARRAYS_NAME = "turb_s0turb_anomaly_arrays.npz"
...
_EXPECTED_DIAGNOSTICS = (
    ...
    f"{WESTERN_BAND_KEY}_rms_and_boundary_to_interior_ratio",
)
...
def training_mean_streamfunction(
    ...
    *,
    ...
    construction :func:`turbfno.validation.train_only_climatology` uses for its
    ...
    *,
    ...
    if pending:
        raise AnomalyContractError(
            ...
            + ", ".join(pending)
            + " -- run `python -m turbfno.anomaly finalize` first"
        )
...
def day2000_structure_summary(
    ...
    boundary = western_boundary_mask(wet, WESTERN_BOUNDARY_WIDTH)
...
def _plot_anomaly_grid(
    ...
    # As in figure 3: the difference panel has its own colorbar and its own
    # scale. Figure 7a below deliberately shares one bound across all three.
...
def _plot_anomaly_long(
    ...
    mean_image = axis.pcolormesh(
        ...
        cmap="RdBu_r",
        vmin=-STREAMFUNCTION_BOUND_SV, vmax=STREAMFUNCTION_BOUND_SV,
        shading="auto",
    )
    axis.set_aspect("equal")
    axis.set_facecolor("0.86")
...
def _readme(report: Mapping[str, Any]) -> str:
    ...
    boundary = structure[WESTERN_BAND_KEY]
