...
EXPECTED_PARAMETER_COUNT = 104_368_296
...
class ProductionArchitecture:
    """The frozen 1-in / 1-out 64x64 operator.
    ...
    grid_shape: tuple[int, int] = (248, 248)
    ...
    def __post_init__(self) -> None:
        ...
        if self.grid_shape != (248, 248) or self.n_modes != (64, 64):
            raise ArchitectureError(
                "the turbulent arm keeps the 248x248 grid and the 64x64 Y,X modes"
            )
        if self.n_layers != 3:
    ...
    class BirePositionalEncoding(nn.Module):
        ...
        def forward(self, value: Any) -> Any:
            if value.ndim != 4 or value.shape[-2:] != (self.height, self.width):
                raise ValueError(
                    f"the Bire position encoder expects N,C,{self.height},{self.width}"
                )
            batch = value.shape[0]
            return torch.cat(
                (
...
class ProductionStepper:
    ...
    normalized block is built once by :func:`turbfno.dataset.static_block` and
