...
:mod:`turbfno.pressure_gradient`, :mod:`turbfno.continuity` and
:mod:`turbfno.barotropic_transport`, and all are joined in
...
reparameterization in :mod:`turbfno.spectral_norm`, so the objective is left
...
#: The spectral loss bins by radius normalized to Nyquist, so a fixed bin
#: count is a fixed *fraction* of the resolved spectrum.  Nyquist is four
#: times higher here than on the 1-degree grid, so 12 bins would each span
#: four times the physical wavenumber band they did there; 48 keeps the
#: physical bin width, and the 240x240 wet rectangle leaves ~600 samples
#: in every bin.
SPECTRAL_BINS = 48

#: The western-boundary band, in cells.  Four cells was four degrees on the
#: 1-degree grid; sixteen keeps the same physical band, which matters because
#: the boundary current is now resolved rather than parameterized by it.
WESTERN_BOUNDARY_WIDTH = 16
...
def loss_contract(config: ProductionLossConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "spectral": (
            "equal_mean_group_48_bin_amplitude_relative_l2_of_10_day_increments_"
        ...
        "boundary": (
            ...
            "on_first_16_wet_cells_east_of_western_wall"
        ),
        "pressure_gradient": (
