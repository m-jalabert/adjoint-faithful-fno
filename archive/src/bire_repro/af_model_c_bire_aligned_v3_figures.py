"""Bire-style Figure 3--8 suites on the trajectory-v3 test block.

Publishes the six figures for the pooled v3 model on each regime's held test
block, using the frozen plotting functions from
:mod:`af_model_c_bire_s0_figures` so the figure definitions, axes, reductions,
and filenames are identical to every earlier package.  S0 is the primary
Bire-style comparison; S1 and S2 are reported as wind-regime robustness.

Three things differ from the frozen runner, all forced by the new data:

* **truth needs no continuation.**  The frozen runner stitches the v2 store to a
  separate evaluation-only MITgcm run because a 2,000-day rollout ran past the
  end of the record.  Trajectory-v3 carries 2,520 test days per regime, so every
  lead is read directly from the store and ``ContinuousS0Truth`` is not used.
* **starts are a new fixed draw.**  The v2 suite's 15 S0 starts (6660--7199)
  index a different record; on v3 they would land inside the *validation* block.
  Starts are redrawn from the declared test window 6480--6999, the only range
  admitting a complete 2,000-day rollout, with a declared RNG seed.
* **Figure 6's comparator changes.**  The prior residual Model C was trained on
  v2 with a v2 normalizer and branch-based S1/S2, so running it on v3 would
  compare two different data worlds.  The black curve is instead this run's own
  earliest checkpoint (step 1,920) against the selected checkpoint (step 7,680)
  --- a training-progress comparison on identical data.  This is labelled in the
  README and the report rather than left to look like the frozen pairing.
...
from .af_model_c_bire_aligned_v3 import train_only_climatology
...
VERSION = "model_c_bire_aligned_v3_figures_v1"
CONTRACT_STATUS = "frozen_after_the_v3_training_and_validation_and_before_any_test_metric"
...
class V3FigureError(RuntimeError):
    """Raised when the trajectory-v3 figure contract is violated."""


def declared_test_starts() -> np.ndarray:
    """15 fixed test starts, drawn once from the declared 2,000-day window."""

    low, high = TEST_START_WINDOW
    candidates = np.arange(low, high, dtype=np.int64)
    draw = np.random.default_rng(START_SEED).choice(
        candidates, size=MEMBER_COUNT, replace=False
    )
    starts = np.sort(draw)
    if int(starts[-1]) + max(LEAD_DAYS) >= 9000:
        raise V3FigureError("a declared start cannot complete a 2,000-day rollout")
    return starts
...
def load_contract(
    ...
    *,
    ...
    if (
        ...
        or tuple(protocol.get("start_draw_order", ())) != tuple(int(v) for v in declared_test_starts())
        ...
        or tuple(protocol.get("regimes", ())) != EXPERIMENTS
        ...
        raise V3FigureError("trajectory-v3 figure contract changed")
    if verify_sources:
        ...
        for relative, expected in contract["source_hashes"].items():
            ...
            if not source.is_file() or _file_sha256(source) != expected:
                raise V3FigureError(f"source changed: {relative}")
...
def _stepper(
    ...
    if (
        ...
        raise V3FigureError(f"{key} identity, dataset, or objective changed")
...
def run(contract_path: str | Path, *, device_name: str = "auto") -> dict[str, Any]:
    ...
    if climatology_days != TRAIN_RANGE[1] - TRAIN_RANGE[0]:
        raise V3FigureError("the v3 train-only climatology did not cover 0--5039")
    ...
    figures._style()
    for regime_index, regime in enumerate(EXPERIMENTS):
