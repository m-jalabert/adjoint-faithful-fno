"""Usual S0 Figures 3--8 for the pressure-gradient-loss fine-tune.

Evaluation is intentionally unchanged from :mod:`oceanfno.figures`.  The only
comparison change is that both selected and comparator use the physical-static
2-in/1-out architecture: black is the retained parent, red is its loss-only
pressure-gradient fine-tune.
"""
...
def load_contract(path: str | Path, *, verify_sources: bool = True) -> tuple[dict[str, Any], Path, str]:
    ...
    if any(_read(contract, tuple(name.split("."))) in (None, PENDING) for name in pending):
        raise PressureGradientFigureError("run figures_pressure_gradient finalize after training")
    if (
