...
Entry points::
    ...
    python -m oceanfno.train preflight --contract config/...json
    python -m oceanfno.train run       --contract config/...json [--device cuda]
"""
...
VERSION = "model_c_production_1in_1out_spectralnorm_v1"

SLUG = "model_c_production_1in_1out_spectralnorm_v1"
...
CONTRACT_STATUS = "frozen_before_any_production_metric"

ROLLOUT_STEPS = 6

#: The effective batch. It is realized as ``MICROBATCH_SIZE`` samples
#: accumulated ``GRADIENT_ACCUMULATION_STEPS`` times, which is arithmetically
#: identical here --- every loss term is a per-sample quantity averaged over the
#: batch, and the microbatches are exactly equal in size because 5,940 training
#: starts per regime divide by four without remainder.
BATCH_SIZE = 8
...
def preflight(contract_path: str | Path) -> dict[str, Any]:
    ...
    if torch is not None:
        ...
        pressure, continuity = physics_contexts(
            group,
            np.zeros((STATE_CHANNEL_COUNT, 62, 62), dtype=np.float32),
            np.ones((STATE_CHANNEL_COUNT, 62, 62), dtype=np.float32),
