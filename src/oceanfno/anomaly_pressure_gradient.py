"""Unchanged anomaly diagnostics for the pressure-gradient-loss figure package."""
...
from . import anomaly as base
from . import figures_pressure_gradient as pgfig
from . import plots
...
def finalize(contract_path: str | Path) -> dict[str, Any]:
    ...
    # The figure contract itself is also finalized by the figure job, so pin its
    # post-finalize bytes together with report/arrays/manifest.
...
def load_contract(path: str | Path, *, verify_sources: bool = True) -> tuple[dict[str, Any], Path, str]:
    ...
    if any(
        ...
        raise PressureGradientAnomalyError("run anomaly_pressure_gradient finalize after figures")
    digests = _sealed(contract)
    for key, digest in digests.items():
