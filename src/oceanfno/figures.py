...
Entry points::
    ...
    python -m oceanfno.figures finalize  --contract config/...json
    python -m oceanfno.figures preflight --contract config/...json
    python -m oceanfno.figures run       --contract config/...json [--device cuda]
"""
...
VERSION = "model_c_production_1in_1out_spectralnorm_v1_s0_figures_v1"
...
_EXPECTED_BASELINES = {
    "climatology": "pointwise_S0_mean_over_the_training_block_0_5999_only",
    ...
    *,
    ...
    if pending:
        raise FigureContractError(
            ...
            + ", ".join(pending)
            + " -- run `python -m oceanfno.figures finalize` first"
        )
