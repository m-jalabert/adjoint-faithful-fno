"""Streamfunction-anomaly companions for the two-in / one-out S0 figures."""
...
from .runtime import _file_sha256, _json_sha256
...
def training_mean_streamfunction(
    ...
    *,
    ...
    if count != stop - start:
        raise ModelCTwoInAnomalyError(
            f"the training mean covered {count} days, not {stop - start}"
        )
    mean = (total / count).astype(np.float32)
    mean[~wet] = 0.0
    if not np.all(np.isfinite(mean)):
        raise ModelCTwoInAnomalyError("the time-mean streamfunction is not finite")
    return mean, count


def _figure_paths(contract: Mapping[str, Any]) -> tuple[Path, Path, Path, Path]:
    artifacts = contract.get("artifacts", {})
    figure_contract = Path(
        str(artifacts.get("figure_package_contract", {}).get("path", ""))
    ).resolve()
    report = Path(
        str(artifacts.get("figure_package_report", {}).get("path", ""))
    ).resolve()
    arrays = Path(
        str(artifacts.get("figure_package_arrays", {}).get("path", ""))
    ).resolve()
    manifest = Path(
        str(artifacts.get("figure_package_manifest", {}).get("path", ""))
    ).resolve()
    return figure_contract, report, arrays, manifest


def _sealed_figure_provenance(contract: Mapping[str, Any]) -> dict[str, str]:
    """Verify the exact two-input contract/report/arrays chain and sealed manifest."""
    ...
    ) != expected_names or not all(
        path.is_file()
        for path in (figure_contract_path, report_path, arrays_path, manifest_path)
    ):
        raise ModelCTwoInAnomalyError(
            "the sealed two-input figure package is incomplete"
        )

    digests = {
        "figure_package_contract": _file_sha256(figure_contract_path),
        "figure_package_report": _file_sha256(report_path),
        "figure_package_arrays": _file_sha256(arrays_path),
        "figure_package_manifest": _file_sha256(manifest_path),
    }
    figure_contract = json.loads(figure_contract_path.read_text())
    report = json.loads(report_path.read_text())
    manifest = json.loads(manifest_path.read_text())
    records = contract["artifacts"]
    expected_arrays_path = (
        Path(figure_contract["output"]["project_root"]).resolve()
        / REGIME
        / plots.ARRAYS_NAME
    )
    expected_report_path = expected_arrays_path.with_name(plots.REPORT_NAME)
    ...
    if (
        ...
        or Path(str(report.get("contract", ""))).resolve() != figure_contract_path
        or report.get("contract_sha256") != digests["figure_package_contract"]
        or report.get("arrays_sha256") != digests["figure_package_arrays"]
        or expected_arrays_path != arrays_path
        or expected_report_path != report_path
        or manifest_path != report_path.parent / plots.MANIFEST_NAME
        ...
        or manifest.get("report_content_sha256")
        ...
        or records["dataset_metadata"].get("path")
        ...
        raise ModelCTwoInAnomalyError("the sealed two-input figure provenance changed")

    with np.load(arrays_path) as stored:
        if not _REQUIRED_ARRAYS.issubset(stored.files):
            raise ModelCTwoInAnomalyError("the two-input figure arrays are incomplete")
        if (
            ...
            raise ModelCTwoInAnomalyError("the two-input figure array protocol changed")
    return digests


def finalize(contract_path: str | Path) -> dict[str, Any]:
    """Fill only deferred figure report/array/manifest hashes, idempotently."""

    resolved = Path(contract_path).resolve()
    contract = json.loads(resolved.read_text())
    digests = _sealed_figure_provenance(contract)
    pinned_contract = contract["artifacts"]["figure_package_contract"].get(
        "sha256"
    )
    if pinned_contract != digests["figure_package_contract"]:
        raise ModelCTwoInAnomalyError(
            "the pinned two-input figure contract changed before finalization"
        )
...
def load_contract(
    ...
    *,
    ...
    """Load the frozen two-input anomaly declaration and its figure provenance."""
    ...
    if pending:
        raise ModelCTwoInAnomalyError(
            "the two-input anomaly contract still carries figure fields: "
            + ", ".join(pending)
            + " -- run `finalize` first"
        )
    ...
    if (
        ...
        or tuple(protocol.get("figure3_lead_days", ()))
        != tuple(plots.FIGURE_3_LEADS)
        or tuple(protocol.get("figure7_lead_days", ()))
        ...
        or output.get("project_root") != _EXPECTED_PROJECT_ROOT
        or output.get("scratch_root") != _EXPECTED_SCRATCH_ROOT
        ...
        raise ModelCTwoInAnomalyError("the two-input anomaly figure contract changed")

    digests = _sealed_figure_provenance(contract)
    for key, digest in digests.items():
        if contract["artifacts"][key].get("sha256") != digest:
            raise ModelCTwoInAnomalyError(f"{key} changed after finalization")
    if verify_sources:
        for label, specification in contract.get("artifacts", {}).items():
            ...
            if not target.is_file() or _file_sha256(target) != specification.get(
                ...
                raise ModelCTwoInAnomalyError(f"{label} changed on disk")
        hashes = contract.get("source_hashes", {})
        if not _REQUIRED_SOURCE_HASHES.issubset(hashes):
            raise ModelCTwoInAnomalyError(
                "the two-input anomaly source declaration is incomplete"
            )
        ...
        for relative, expected in hashes.items():
            ...
            if not source.is_file() or _file_sha256(source) != expected:
                raise ModelCTwoInAnomalyError(
                    f"two-input anomaly source changed: {relative}"
                )
...
def _normalized_first_difference(
    ...
    *,
    ...
    if axis not in (0, 1):
        raise ValueError("streamfunction roughness axis must be 0 or 1")
    ...
    if scale == 0.0 or not np.any(valid):
        return float("nan")
    difference = np.asarray(field, dtype=np.float64)[pair] - np.asarray(
...
def _directional_high_wavenumber_fraction(
    ...
    *,
    ...
    return {
        "meridional": float(power[np.broadcast_to(np.abs(fy) > threshold, power.shape)].sum() / total),
...
{gradients['model_meridional']:.3f} meridionally and
{gradients['model_zonal']:.3f} zonally; directional high-wavenumber fractions
    ...
    try:
        ...
        report = {
            ...
            "day2000_structure": day2000_structure_summary(
                anomalies["figure7_truth"][-1],
                anomalies["figure7_model"][-1],
                wet,
            ),
            "total_field_note": (
