...
* the **normalization**, computed from the training block alone: a pointwise
  ``(46, 62, 62)`` mean and scale, and the per-channel RMS of the normalized
...
def assert_store_is_v3(group: Any) -> None:
    ...
    The v3 store carries the project's own three-block split. This protocol
    ...
    if str(group.attrs.get("version")) != DATASET_VERSION:
        raise BireProtocolSplitError("the store is not trajectory-v3")
    stored = np.asarray(group["snapshot_split"][:], dtype=np.uint8)
    if stored.size != STORE_DAYS:
        raise BireProtocolSplitError("the v3 store has an unexpected record length")
