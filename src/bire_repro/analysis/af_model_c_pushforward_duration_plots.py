"""Project-facing outputs for the immutable Model C duration experiment."""
...
from ..af_model_c_pushforward_duration import (
    ARRAYS_NAME,
    DURATION_VERSION,
    REPORT_NAME,
)
...
PLOT_VERSION = "model_c_pushforward_duration_plots_v1"
FIGURE_NAME = "model_c_pushforward_duration_sst_phihyd_rmse.png"
CSV_NAME = "model_c_pushforward_duration_sst_phihyd_rmse.csv"
SUMMARY_NAME = "pushforward_duration_summary.json"
...
class ModelCPushforwardDurationPlotError(RuntimeError):
    """Raised when duration publication provenance is invalid."""
...
def load_duration_evidence(
    report_path: str | Path,
    arrays_path: str | Path,
) -> tuple[dict[str, Any], Mapping[str, np.ndarray]]:
    """Load and verify one complete, sealed duration result."""
    ...
    if (
        ...
        or report.get("version") != DURATION_VERSION
        ...
        raise ModelCPushforwardDurationPlotError(
            "expected a complete, sealed, replay-exact duration experiment"
        )
    content = dict(report)
    expected_hash = content.pop("report_content_sha256", None)
    if expected_hash != _json_sha256(content):
        raise ModelCPushforwardDurationPlotError(
            "duration report content hash changed"
        )
    if _file_sha256(arrays_path) != report.get("arrays_sha256"):
        raise ModelCPushforwardDurationPlotError("duration arrays changed")
...
def mean_rmse_curve(
    ...
    if method in BASELINES:
        prefix = f"source_source_{method}"
    elif method == "pushforward_v1":
        prefix = "source_fine_tune_1920"
    else:
        prefix = f"duration_{int(method)}"
    key = f"{prefix}__rmse__{field}"
    values = np.asarray(arrays[key], dtype=np.float64)
    if values.ndim != 2 or values.shape[1] != len(LEAD_DAYS):
        raise ValueError(f"invalid duration curve {key}")
...
def publication_rows(
    ...
    for field in FIELDS:
        ...
        methods = (*BASELINES, "pushforward_v1", *(str(step) for step in steps))
        for method in methods:
            curve = mean_rmse_curve(arrays, method, field)
            if method in BASELINES:
                published_method = method
            elif method == "pushforward_v1":
                published_method = "fine_tune_1920"
            else:
                published_method = f"fine_tune_{method}"
...
def _plot(
    ...
    for row, field in enumerate(FIELDS):
        ...
        source = mean_rmse_curve(arrays, "pushforward_v1", field)
        raw_axis.plot(
            leads,
            source,
            color="#7A7A7A",
            linewidth=1.6,
            label="Pushforward-v1 step 1,920",
        )
        ratio_axis.plot(
            leads,
            source / persistence,
        ...
        for color, step in zip(colors, steps):
            ...
            label = (
                f"Duration {step} (diagnostic best)"
                if step == selected_step
                else f"Duration {step}"
            )
            raw_axis.plot(
                leads,
                curve,
    ...
    axes[0, 0].legend(ncol=2, fontsize=6.5, loc="upper left")
    figure.suptitle(
        "Model C pushforward-v1 low-rate duration diagnosis "
...
def _summary(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    decision = report["duration_decision"]
    return {
        "status": "complete",
        "classification": decision["classification"],
        "passed": bool(decision["passed"]),
        "selected_fine_tune_step": int(
            decision["selected_fine_tune_step"]
        ),
        "selected_for": decision["selected_for"],
        "pushforward_v1_replay_bitwise_exact": True,
...
def generate_duration_outputs(
    ...
    if output.exists() or temporary.exists():
        raise FileExistsError(
            f"refusing to overwrite duration outputs: {output}"
        )
    report, arrays = load_duration_evidence(report_path, arrays_path)
    steps = [
    ...
    try:
        ...
        (temporary / README_NAME).write_text(
            "# Model C pushforward duration diagnosis\n\n"
            f"Classification: `{summary['classification']}`.\n\n"
            "The figure and CSV compare pushforward-v1 step 1,920 and all "
            "eight duration checkpoints with persistence and training "
            "climatology. Solid ratio curves use persistence and dotted "
            "curves use climatology; values below one win.\n\n"
            "The original pushforward path replayed bitwise exactly. This "
            "experiment used split 1 only and did not authorize replication "
            "or validation.\n"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = generate_duration_outputs(
        args.report,
        args.arrays,
        args.output_dir,
    )
