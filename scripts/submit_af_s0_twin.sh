#!/usr/bin/env bash
# Submit the three restart-chained segments of the MITgcm S0 twin experiment:
# years 100-110 (perturbed start), 110-120, and 120-125.
set -euo pipefail
...
for segment in "100:10" "110:10" "120:5"; do
  ...
  if [[ -z "${parent_id}" ]]; then
    job_id="$(sbatch --parsable --export="${exports}" slurm/mitgcm/af_s0_twin_segment.sbatch)"
  else
    job_id="$(sbatch --parsable --dependency="afterok:${parent_id}" --export="${exports}" \
      slurm/mitgcm/af_s0_twin_segment.sbatch)"
  fi
  printf 'S0 twin years %03d-%03d: %s\n' "${start_year}" "$((start_year + years))" "${job_id}"
  parent_id="${job_id}"
done
