...
from bire_repro.analysis.af_forward_compare import _check_common_protocol
...
def test_training_climatology_keeps_regimes_separate() -> None:
    state = np.zeros((3, 5, 46, 3, 3), dtype=np.float32)
    ...
    means, derived, count = _training_climatology(
        state, np.array([1, 1, 0, 1, 1], dtype=np.uint8), wet, chunk=3
    )
    assert count == 4
