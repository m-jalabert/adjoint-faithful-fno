...
CONTRACT = ROOT / "config/model_c_bire_protocol_rollout_ft_v1.json"
PARENT = ROOT / "config/model_c_bire_protocol_duration_v1.json"
SBATCH = ROOT / "slurm/models/c/af_model_c_bire_protocol_rollout_ft.sbatch"
...
def test_the_rollout_term_is_not_the_three_step_one() -> None:
    ...
    from bire_repro.af_model_c import model_c_loss_terms
...
def test_outputs_do_not_collide_with_the_parent_package() -> None:
    ...
    for key in ("scratch_root", "project_root"):
        ...
        assert contract["output"][key].endswith("bire_protocol_rollout_ft_v1")
...
def test_six_step_starts_keep_the_whole_target_sequence_inside_training() -> None:
    from bire_repro.af_bire_protocol_split import store_codes
...
def test_artifact_names_do_not_collide_with_the_parents() -> None:
    from bire_repro import af_model_c_bire_protocol_duration as duration
    from bire_repro import af_model_c_bire_protocol_rollout_ft as arm
    ...
    for name in ("REPORT_NAME", "ARRAYS_NAME", "FIGURE_NAME",
        ...
        assert getattr(arm, name) != getattr(duration, name)
        assert "rollout_ft" in getattr(arm, name)
    names = {f"{arm.CHECKPOINT_STEM}_{s:05d}.pt" for s in CHECKPOINT_STEPS}
    others = {f"{duration.CHECKPOINT_STEM}_{s:05d}.pt" for s in duration.CHECKPOINT_STEPS}
    others |= {f"{protocol.CHECKPOINT_STEM}_{s:05d}.pt" for s in protocol.CHECKPOINT_STEPS}
...
def test_launcher_invokes_its_own_module_and_contract() -> None:
    ...
    assert invoked == {"bire_repro.af_model_c_bire_protocol_rollout_ft"}
    assert "model_c_bire_protocol_rollout_ft_v1.json" in text
    assert "model_c_bire_protocol_duration_v1.json" not in text


def test_the_arm_does_not_bind_itself_into_the_parents_training_loop() -> None:
    """Importing this arm must leave the parent module exactly as it found it."""

    from bire_repro import af_model_c_bire_protocol_rollout_ft as arm

    assert not hasattr(arm, "_ParentBinding")
    assert protocol.VERSION == "model_c_bire_protocol_pooled_v1"
    assert protocol.ROLLOUT_STEPS == 3
    assert protocol.CHECKPOINT_STEPS == (1920, 3840, 5760, 7680)
    assert protocol.load_contract.__module__ == protocol.__name__
