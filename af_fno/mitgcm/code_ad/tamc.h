...
C--   AF--FNO adjoint checkpointing.
C
C     AUTODIFF_CHECK enforces nchklev_1*nchklev_2*nchklev_3 >= nTimeSteps.
C     With deltaT = 1200 s there are 72 steps per model day, so
C
C         72 * 200 * 1 = 14400 steps = 200 model days.
C
C     Sized for 200 days, not for the 10-day and 20-day production runs of
C     the first experiment.  These three integers reach TAF through the
C     "CADJ INIT comlev1 = COMMON,nchklev_1" tape-sizing directives in
C     the_main_loop.F, so raising them later costs a new TAF submission.
C     nchklev_2/nchklev_3 cost scratch disk, not memory or licence, so the
C     headroom is free.
C
C     nchklev_1 = 72 (one model day) sets the in-memory level-1 tape.  At
C     31x31x15 per rank on 4 ranks this is a few hundred MB per rank.
C     Levels 2 and 3 go to disk via ALLOW_AUTODIFF_WHTAPEIO.
      integer    nchklev_1
