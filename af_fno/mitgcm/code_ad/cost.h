...
#ifdef ALLOW_COST_TEST
C--   AF--FNO: the quantity of interest is a runtime weight field.
C     cost_test.F computes  J = sum_ij w_ij * etaN_ij(T)  with w read from
C     afCostWeightFile, so p*, the wet-mean convention and any other linear
C     functional of the final SSH are runtime choices.  A passive character
C     variable: it never enters the differentiated dependency chain.
      COMMON /AF_COST_WEIGHT_C/ afCostWeightFile
      CHARACTER*(MAX_LEN_FNAM) afCostWeightFile
#endif
