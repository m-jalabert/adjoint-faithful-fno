...
\end{decisionbox}
\vfill
{\large Revised technical proposal --- 27 July 2026\par}\vspace{0.45in}
...
The project now separates two claims.  The \emph{adjoint-readiness} forward gate is
...
anomaly-scale drift.  Job 292293 now supplies the required training-only parent:
at days 10/30 its SST ratios are 0.541/0.601 to persistence and
0.293/0.693 to climatology; surface-\(P/\rho\), surface speed, and streamfunction
also beat both baselines at both horizons.  The next contracted action is to freeze
this short gate, its damped-persistence comparison, ensemble rule, and anomaly-scale
bound before any new validation metric is read.  Passing that versioned 10--30-day gate may open the
response/adjoint campaign without asserting 90-day autoregressive stability.  The
\emph{long-rollout} gate below remains mandatory for a 90--360-day emulator claim and
continues in parallel; it is no longer the sole serial blocker for M7--M10.  Until the
short-horizon amendment is frozen and passed, all later archives remain sealed.
